...
def normal_smoothing_pos(positive_words_dict, total_positive_count, total_negative_count, total_words):
    pos_words_prob = {}
    Mu = 0.39
    for i in positive_words_dict.keys():
        pos_words_prob[i] = ((positive_words_dict[i] + (Mu * (total_words[i] / (
                total_negative_count + total_positive_count)))) / (total_positive_count + Mu))
    return pos_words_prob


def normal_smoothing_neg(negative_words_dict, total_negative_count, total_positive_count, total_words):
    neg_words_prob = {}
    Mu = 0.39
    for i in negative_words_dict.keys():
        neg_words_prob[i] = ((negative_words_dict[i] + (Mu * (total_words[i] / (
                total_negative_count + total_positive_count)))) / (total_negative_count + Mu))
...
def main(inputdir, output):
    ...
    neg_words_prob = normal_smoothing_neg(negative_words_dict, total_negative_count, total_positive_count, total_words)
    pos_words_prob = normal_smoothing_pos(positive_words_dict, total_positive_count, total_negative_count, total_words)
